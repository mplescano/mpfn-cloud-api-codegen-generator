...
public class MpfnSpringGeneratorCodegenTest {
	...
	public void testParameterOrdersUseOas2() throws Exception {
		...
		final CodegenConfigurator configurator = new CodegenConfigurator().setLang("mpfn-spring")
	...
	@Test
	public void testParameterSchemaObjectExplode() throws Exception {
		final TemporaryFolder folder = new TemporaryFolder();
		folder.create();
		final File output = folder.getRoot();

		final CodegenConfigurator configurator = new CodegenConfigurator().setLang("mpfn-spring")
				.setInputSpecURL("src/test/resources/3_0_0/parameterSchemaObjectExplode.yaml")
		...
		Assert.assertTrue(content.contains(
				"ResponseEntity<BaseRecordVersion> getTest(@Parameter(in = ParameterIn.QUERY, description = \"query description\" ,schema=@Schema()) @Valid  BaseAudit name)"),
				content);
